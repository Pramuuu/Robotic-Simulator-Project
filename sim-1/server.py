        ...
        async for message in websocket:
            ...
            try:
                ...
                if isinstance(data, dict) and data.get("type") == "collision" and data.get("collision"):
                    collision_count += 1
                # Forward all JSON simulator messages to every connected client
                # (controllers can listen for capture_image_response, goal_reached, etc.)
                if isinstance(data, dict):
                    for ws in list(connected):
                        try:
                            asyncio.run_coroutine_threadsafe(ws.send(json.dumps(data)), async_loop)
                        except Exception:
                            pass
            except Exception:
                pass
